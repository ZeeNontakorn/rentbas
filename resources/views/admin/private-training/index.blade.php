    ...
    <div class="container mx-auto px-6 max-w-6xl">
        ...
        <div class="flex gap-2 -mb-px overflow-x-auto select-none border-b border-gray-200 mb-6">
            @foreach([
                ...
                <a href="{{ route('admin.private-training.index', ['status' => $tab['key']]) }}"
                    class="px-5 py-2.5 text-sm font-medium border-b-2 transition-all cursor-pointer {{ $status === $tab['key'] ? 'border-orange-500 text-orange-600 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    {{ $tab['label'] }}
                </a>
            @endforeach
        </div>
                        ...
                        <div class="flex justify-between items-start flex-wrap gap-3">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-xs px-2 py-0.5 border border-gray-300 rounded">โค้ช {{ $b->coach->name }}</span>
                                ...
                                <p class="text-sm font-medium text-gray-800">ลูกค้า: {{ $b->user->name }} ({{ $b->user->email }})</p>
                                <p class="text-xs text-gray-500 mt-1">
                                    วันที่ {{ \Carbon\Carbon::parse($b->date)->format('d/m/Y') }}
                                    &nbsp;•&nbsp; เวลา {{ substr($b->start_time, 0, 5) }} - {{ substr($b->end_time, 0, 5) }} น.
                                </p>
                                @if($b->note)
                            ...
                            @if($b->status === 'pending')
                                <div class="flex gap-2">
                                    <button type="button" onclick="openRejectModal('{{ route('admin.private-training.reject', $b) }}')"
                                    ...
                                    <form method="POST" action="{{ route('admin.private-training.approve', $b) }}">
                                        @csrf
                                        <button type="submit" class="bg-green-500 text-white text-xs px-3 py-1.5 rounded cursor-pointer transition duration-200 hover:scale-105 hover:bg-green-600">อนุมัติ</button>
                                    </form>
                                </div>
                            @endif
                        </div>
...
<div id="rejectModal" class="fixed inset-0 z-[60] hidden bg-gray-900/60 backdrop-blur-sm items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full overflow-hidden border border-gray-100">
        ...
        <form id="rejectForm" method="POST" class="p-6 space-y-4">
            ...
            <div class="flex gap-2">
                ...
                <button type="submit" class="w-1/2 px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg transition font-medium shadow-sm">ยืนยันปฏิเสธ</button>
            </div>
        </form>
    </div>
</div>
