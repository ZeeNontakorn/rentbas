...
{
    /**
     * เพดานราคาสูงสุดต่อแพ็กเกจ (บาท) — กันกรอกเลขหลุด range ของ column total_price ใน DB
     * ใช้ค่าเดียวกันทั้ง server-side validation และ client-side (ส่งผ่าน view)
     */
    public const MAX_PACKAGE_PRICE = 1000000;
    ...
    private function validated(Request $request): array
    {
        $validated = $request->validate([
            ...
            'packages.*.total_price.max' => 'ราคาต้องไม่เกิน '.number_format(self::MAX_PACKAGE_PRICE).' บาท ต่อแพ็กเกจ',
        ]);
